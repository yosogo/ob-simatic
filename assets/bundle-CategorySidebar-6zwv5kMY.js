import { c as createAstro, a as createComponent, m as maybeRenderHead, e as addAttribute, d as renderTemplate } from './bundle-astro/server-iX5mIn4f.js';
import 'piccolore';
import 'clsx';
import { c as categories } from './bundle-i18n-C_ibLEKJ.js';
/* empty css                                */
...
const $$CategorySidebar = createComponent(($$result, $$props, $$slots) => {
  ...
  const topLevelCategories = Object.values(categories).filter((cat) => cat.parentSlug === null).map((cat) => {
    const loc = getCatLoc(cat);
    return {
      slug: cat.slug,
      name: loc.name,
  ...
  parentSlug ? Object.values(categories).filter((cat) => cat.parentSlug === parentSlug).map((cat) => {
    const loc = getCatLoc(cat);
    return {
      slug: cat.slug,
      name: loc.name,
  ...
  const childCategories = Object.values(categories).filter((cat) => cat.parentSlug === currentCategorySlug).map((cat) => {
    const loc = getCatLoc(cat);
    return {
      slug: cat.slug,
      name: loc.name,
...
}, "C:/Users/muhammed/Desktop/code/anode/project/src/components/CategorySidebar.astro", void 0);
